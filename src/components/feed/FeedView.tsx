...
const mapAppRoleToFeedRole = (role: AppRole | undefined): FeedPost["role"] => role ?? "resident";
              ...
              <Card className="mb-6 p-4 border-2 border-earth-green/40 bg-gradient-to-br from-white via-warm-cream to-earth-green-light/10">
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                  <div className="flex gap-2">
                    <Button
                      ...
                      className={
                        photoAttached
                          ? "bg-earth-green/30 text-earth-green hover:bg-earth-green/40"
                          : "text-earth-green hover:bg-earth-green/10"
                      }
                  ...
                  <Button
                    size="sm"
                    className="rounded-full bg-earth-green hover:bg-earth-green/90 text-white"
                    onClick={handlePost}
                ...
                {(photoAttached || locationAdded) && (
                  <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted-foreground">
                    {photoAttached && (
                      <span className="rounded-full bg-earth-green/20 text-earth-green px-2 py-1 font-medium">
                        ✓ Photo attached
                      </span>
                    )}
            ...
            <div className="mb-4 flex flex-wrap items-center gap-2">
              ...
              <div className="ml-auto relative w-full max-w-xs">
                ...
                <Input
                  ...
                  className="pl-9 border-earth-green/30 focus:border-earth-green focus:ring-earth-green/30"
                />
              </div>
            </div>
          ...
          <aside className="space-y-5">
            <Card className="p-5 border-2 border-earth-green/40 bg-gradient-to-br from-white to-earth-green-light/10">
              ...
              <ul className="mt-3 space-y-2 text-sm">
                {(Object.keys(roleMeta) as FeedPost["role"][]).map((r) => (
                  <li
                    key={r}
                    className="flex items-center justify-between p-2 rounded-md hover:bg-earth-green/10 transition-colors"
                  >
                    <span className="text-muted-foreground">{roleMeta[r].label}s</span>

                    <span className="font-semibold text-earth-green">
                      {posts.filter((p) => p.role === r).length}
                    </span>
                  </li>
                ))}
              </ul>
            ...
            <Card className="p-5 border-2 border-earth-green/40 bg-gradient-to-br from-white to-earth-green-light/10">
              ...
              <ul className="mt-3 space-y-1.5 text-sm text-earth-green/80">
                <li className="flex items-center gap-2 p-2 rounded-md hover:bg-earth-green/10 transition-colors cursor-pointer">
                  <span className="text-earth-green">→</span> #barter
                </li>

                <li className="flex items-center gap-2 p-2 rounded-md hover:bg-earth-green/10 transition-colors cursor-pointer">
                  <span className="text-earth-green">→</span> #localharvest
                </li>

                <li className="flex items-center gap-2 p-2 rounded-md hover:bg-earth-green/10 transition-colors cursor-pointer">
                  <span className="text-earth-green">→</span> #zerowaste
                </li>

                <li className="flex items-center gap-2 p-2 rounded-md hover:bg-earth-green/10 transition-colors cursor-pointer">
                  <span className="text-earth-green">→</span> #bayanihan
                </li>
              </ul>
            </Card>
          </aside>
