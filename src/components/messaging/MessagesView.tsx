...
export function MessagesView() {
  ...
  const loadConversations = useCallback(async () => {
    ...
    const conversationsWithProfiles = await Promise.all(
      (convData || []).map(async (conv: any) => {
        ...
        const unreadCount =
          conv.messages?.filter((m: Message) => m.sender_id !== user.id && !m.read_at).length || 0;
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      ...
      const unreadMessages =
        (data as Message[])?.filter((m) => m.sender_id !== user?.id && !m.read_at) || [];
            ...
            prev.map((conv) =>
              conv.id === selectedConversation.id
                ? {
                    ...
                    unread_count:
                      newMsg.sender_id !== user?.id && !newMsg.read_at
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (!selectedConversation || !user || (!newMessage.trim() && selectedImages.length === 0))
      return;
        ...
        try {
          const uploadPromises = imagesToUpload.map(async (file) => {
            ...
            if (!allowedTypes.includes(file.type)) {
              throw new Error(
                `Invalid file type for ${file.name}. Please upload JPEG, PNG, GIF, or WebP images.`,
              );
            }
          ...
          setMessages((prev) =>
            prev.map((msg) =>
              msg.id === newMessageData.id ? ({ ...msg, image_urls: imageUrls } as any) : msg,
            ),
          );
          ...
          if (
            ...
            toast.error(
              `Storage bucket "${STORAGE_BUCKET}" not found. Please create it in Supabase Storage.`,
            );
          } else if (
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center justify-between">
                            <p className="text-sm text-slate-600 truncate">
                              {conv.last_message
                                ? (conv.last_message as any)?.image_urls &&
                        ...
                        >
                          ...
                          >
                            {console.log(
                              "Rendering message:",
                              message.id,
                              (message as any).image_urls,
                            )}
                            {(message as any).image_urls &&
                              (message as any).image_urls.length > 0 && (
                      ...
                      {imagePreviews.map((preview, index) => (
                        <div key={index} className="relative inline-block">
                          <img
                            src={preview}
                            alt={`Preview ${index + 1}`}
                            className="h-20 w-20 object-cover rounded-xl border-2 border-emerald-200 shadow-md"
                          />
                          <Button
                            type="button"
                            size="sm"
                  ...
                  <form onSubmit={handleSendMessage} className="flex gap-3 items-center">
                    <div className="relative">
                      ...
                      <Button
                        ...
                        variant="outline"
                        disabled={sending || uploadingImage}
                        className="h-12 w-12 rounded-full border-slate-300 hover:border-emerald-500 hover:bg-emerald-50 transition-all"
                      >
                    ...
                    <Button
                      ...
                      className="h-12 w-12 rounded-full bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white shadow-md shadow-emerald-500/30 transition-all"
                    >
                      {uploadingImage ? (
