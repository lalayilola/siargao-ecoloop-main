      ...
      <div className="flex items-center justify-between gap-2 overflow-x-auto pb-2">
        {steps.map((step, index) => (
          <div key={step.label} className="flex flex-col items-center min-w-[100px]">
            ...
            {index < steps.length - 1 && <ArrowRight className="h-4 w-4 text-primary/50 mt-1" />}
          </div>
        ))}
      </div>
      <div className="mt-4 text-center">
        <p className="text-xs text-slate-600">
          <span className="font-semibold text-primary">Zero Waste Program:</span> Converting food
          waste into compost for sustainable agriculture
        </p>
      </div>
    </Card>
  );
