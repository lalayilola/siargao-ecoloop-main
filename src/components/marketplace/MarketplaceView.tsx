...
export function MarketplaceView() {
  ...
  const [filterUserRole, setFilterUserRole] = useState<
    "farmer" | "restaurant" | "resident" | "lgu_admin" | "all"
  >("all");
  ...
  useEffect(() => {
    ...
    const loadStatistics = async () => {
      try {
        ...
        const completedTransactions = purchaseRequests.filter(
        ...
        const wasteDiverted = purchaseRequests
          .filter(
            (request) =>
              request.status === "completed" && listingKindById.get(request.listing_id) === "waste",
          )
  ...
  const createListing = async () => {
    ...
    if (files.length > 0) {
      ...
      try {
        ...
        if (
          ...
          toast.error(
            `Storage bucket "${STORAGE_BUCKET}" not found. Create it in Supabase Storage or set VITE_SUPABASE_STORAGE_BUCKET.`,
          );
    ...
    const { data, error } = await (supabase.from("marketplace_listings") as any)
      ...
      .insert({
        ...
        location_address:
          listingLocationAddress.trim() || selectedLocation?.locationAddress || null,
  ...
  const handlePurchaseRequestStatus = async (
    requestId: string,
    status: Database["public"]["Enums"]["trade_status"],
  ) => {
    const { data: existingRequest, error: existingRequestError } = await (
      supabase.from("purchase_requests") as any
    )
    ...
    const { data: updatedRequest, error } = await (supabase.from("purchase_requests") as any)
    ...
    if ((status === "accepted" || status === "completed") && updatedRequest) {
      ...
      if (shouldDeduct) {
        const listing =
          listings.find((item) => item.id === updatedRequest.listing_id) ??
          userListings.find((item) => item.id === updatedRequest.listing_id);

        if (listing) {
          const requestedQty = Math.max(
            1,
            Number(updatedRequest?.quantity_kg ?? existingRequest?.quantity_kg ?? 1) || 1,
          );
          const currentKg = Number(listing.kg ?? 0);
          const nextKg = Math.max(0, currentKg - requestedQty);

          const { error: stockError } = await (supabase.from("marketplace_listings") as any)
          ...
          setListings((prev) =>
            prev.map((item) => (item.id === listing.id ? { ...item, kg: nextKg } : item)),
          );
    ...
    () =>
      (kind: Listing["kind"], source: "all" | "restaurant" | "lgu_admin" = "all") =>
        listings
          ...
          .filter((l) =>
            q
              ? `${l.title} ${l.seller} ${l.barangay}`.toLowerCase().includes(q.toLowerCase())
              : true,
          )
          ...
          .filter(
            (l) =>
              !filterCategory || l.category?.toLowerCase().includes(filterCategory.toLowerCase()),
          )

          .filter(
            (l) =>
              !filterBarangay || l.barangay?.toLowerCase().includes(filterBarangay.toLowerCase()),
          ),
      ...
      <Container className="py-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-amber-700">
                  Upload your government ID in your{" "}
                  <Link
                    to="/profile"
                  ...
                  to get verified by the LGU. Verification is required to create listings, buy, and
                  send messages.
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(
                    value: "farmer" | "restaurant" | "resident" | "lgu_admin" | "all",
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="e.g., vegetables, waste"
        ...
        {!user && (
          <Card className="mb-6 border-2 border-primary/30 p-4 text-center text-sm bg-secondary/10 text-slate-700">
            ...
            <Link to="/auth" className="text-primary underline hover:text-primary/80 font-medium">
              Sign in
            </Link>{" "}
            to message vendors and post your own listings.
          </Card>
        )}
        ...
        {showForm && user && profile?.primary_role !== "resident" && (
          <Card className="mb-6 p-4 border-2 border-primary/30 bg-gradient-to-br from-white to-secondary/10 shadow-sm shadow-primary/10">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="col-span-2">
                ...
                <Input
                  ...
                  placeholder={kind === "produce" ? "e.g., Tomatoes" : "e.g., Food Waste"}
                  className="border-primary/30 focus:border-primary focus:ring-primary/50 h-8 text-xs"
                />
              </div>
            </div>
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              <div>
                <label className="mb-1 block text-[10px] font-medium text-primary">
                  Add photos
                </label>
                  ...
                  {files.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-4">
                      <svg
                        ...
                        className="h-6 w-6 text-primary/60 mb-1"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
            ...
            <div className="mt-2">
              <Button
                ...
                disabled={uploading}
                className="rounded-full bg-gradient-to-r from-primary to-secondary text-white hover:from-primary/90 hover:to-secondary/90 h-8 text-xs"
              >
        ...
        >
          <TabsList
            className={`grid w-full ${profile?.primary_role === "lgu_admin" ? "grid-cols-3" : profile?.primary_role === "farmer" || profile?.primary_role === "restaurant" ? "grid-cols-2" : "grid-cols-1"} bg-sand/50 border-2 border-primary/30 rounded-lg p-1`}
          >
            <TabsTrigger
              value="produce"
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {filtered("produce").map((l) => (
                <ListingCard
                  ...
                  onBuy={
                    canBuyListing(l, profile?.primary_role, profile?.lgu_approved || false)
                      ? () => {
                          setSelectedListing(l);
          ...
          <TabsContent value="waste" className="mt-6">
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {filtered("waste", profile?.primary_role === "lgu_admin" ? "all" : "restaurant").map(
                (l) => (
                  <ListingCard
                    key={l.id}
                    item={l}
          ...
          {(profile?.primary_role === "lgu_admin" || profile?.primary_role === "farmer") && (
            <TabsContent value="compost" className="mt-6">
              <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {filtered(
                  "compost",
                  profile?.primary_role === "lgu_admin" ? "lgu_admin" : "all",
                ).map((l) => (
                  <ListingCard
                    key={l.id}
                    item={l}
        ...
        user={
          user
            ? {
                ...
                full_name: profile?.full_name || "",
                primary_role:
                  (profile?.primary_role === "super_admin" ? "lgu_admin" : profile?.primary_role) ||
                  "resident",
              }
            : null
        }
            ...
            <div className="grid gap-6 bg-background sm:grid-cols-[1.5fr_1fr]">
              <div className="relative overflow-hidden bg-slate-950">
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-slate-950/90 to-transparent p-6">
                  <p className="text-sm uppercase tracking-[0.2em] text-slate-200/70">
                    {selectedListing.kind === "produce" ? "Produce" : "Waste"}
                  </p>
                  ...
                  <p className="mt-2 text-lg font-semibold text-primary">
                    {selectedListing.price ? `₱${selectedListing.price}` : "Free / Barter"}
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-6 p-6">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    {selectedListing.barangay} • {selectedListing.available_at} •{" "}
                    {selectedListing.kg} kg
                  </DialogDescription>
                </DialogHeader>
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        ...
                        <p className="text-sm text-slate-600">
                          {selectedListing.role
                            .replace("hotel_restaurant", "Restaurant")
                            .replace("lgu_admin", "LGU")}
                        </p>
                      </div>
                      ...
                      <div className="text-right">
                        ...
                        <p className="font-semibold text-slate-800">
                          {selectedListing.transaction_type
                            .replace("sell_only", "Sell only")
                            .replace("barter_only", "Barter only")
                            .replace("sell_and_barter", "Sell & barter")}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm leading-relaxed text-slate-700">
                      {selectedListing.category
                        ? `${selectedListing.category.charAt(0).toUpperCase() + selectedListing.category.slice(1)} available in ${selectedListing.barangay}.`
                        : "A marketplace listing from the community."}
                    </p>

                    {selectedListing.acceptable_exchanges &&
                      selectedListing.acceptable_exchanges.length > 0 && (
                          ...
                          <button
                            ...
                            className={`relative rounded-xl overflow-hidden border-2 transition-all hover:border-primary ${
                              index === currentImageIndex
                                ...
                                : "border-slate-200/60 hover:border-primary/60"
                            }`}
                          >
                            <img
                              src={photo}
