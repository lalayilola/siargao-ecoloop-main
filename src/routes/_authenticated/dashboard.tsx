...
export const Route = createFileRoute("/_authenticated/dashboard")({
  head: () => ({ meta: [{ title: "LGU Monitoring Dashboard — Farm2Food Cycle" }] }),
...
function Stat({
  ...
  const displayValue = value.includes("₱")
    ...
    : value.includes("kg")
      ? `${animatedValue.toLocaleString()} kg`
      : animatedValue.toLocaleString();
...
function DashboardPage() {
  ...
  const [vegetableSalesData, setVegetableSalesData] = useState<
  ...
  const loadMonitoringData = useCallback(async () => {
    ...
    try {
      ...
      const [profilesResult, listingsResult, purchaseResult, tradeResult, wasteReportResult] =
        await Promise.all([
      ...
      const wasteReports = (wasteReportResult.data || []) as Array<{
        id: string;
      ...
      const completedTransactions = [
        ...municipalityPurchases.filter((purchase) =>
          ["accepted", "completed"].includes(purchase.status),
        ),
      ...
      const completedProducePurchases = municipalityPurchases.filter(
      ...
      municipalityWasteReports.forEach((report) => {
        ...
        if (restaurant && restaurant.primary_role === "restaurant") {
          ...
          restaurantWasteMap.set(
            restaurantName,
            (restaurantWasteMap.get(restaurantName) || 0) + quantity,
          );
        }
      });
      ...
      const completedCompostPurchases = municipalityPurchases.filter(
        (purchase) => purchase.status === "completed",
      );
    ...
    <>
      ...
      <Container className="py-12 relative">
        ...
        <div className="mb-8 grid gap-4 lg:grid-cols-4">
          <button
            onClick={() =>
              document.getElementById("monitoring-charts")?.scrollIntoView({ behavior: "smooth" })
            }
            ...
            <div className="flex items-center justify-center gap-2 mt-4">
              ...
              <Button
                ...
                onClick={() =>
                  setCurrentPage(
                    Math.min(
                      Math.ceil(vegetableSalesData.length / itemsPerPage) - 1,
                      currentPage + 1,
                    ),
                  )
                }
        ...
        <Card className="mt-8 p-6 rounded-2xl border border-emerald-200 bg-white/80 backdrop-blur-sm shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-display text-lg font-bold text-slate-900">
                Food Waste by Restaurant
              </h3>
              <p className="text-sm text-slate-500">
                Top 10 restaurants by food waste submitted in {monitoringStats.municipalityLabel}
              </p>
            </div>
          </div>
          <div className="mt-4 h-56">
          ...
          {foodWasteData.length > itemsPerPage && (
            <div className="flex items-center justify-center gap-2 mt-4">
              ...
              <Button
                ...
                onClick={() =>
                  setCurrentPage(
                    Math.min(Math.ceil(foodWasteData.length / itemsPerPage) - 1, currentPage + 1),
                  )
                }
        ...
        <Card className="mt-8 p-6 rounded-2xl border border-emerald-200 bg-white/80 backdrop-blur-sm shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-display text-lg font-bold text-slate-900">
                Compost Sales by Product
              </h3>
              <p className="text-sm text-slate-500">
                Top 10 compost products sold in {monitoringStats.municipalityLabel}
              </p>
            </div>
          </div>
          <div className="mt-4 h-56">
          ...
          <Card className="p-6 rounded-2xl border border-emerald-200 bg-white/80 backdrop-blur-sm shadow-sm">
            ...
            <div className="mt-6 space-y-3">
              {membersPreview.length === 0 ? (
                <p className="text-sm text-slate-500">
                  No members recorded for this municipality yet.
                </p>
              ) : (
                membersPreview.map((member) => (
                  <div
                    key={member.id}
                    className="flex items-center gap-4 p-4 rounded-xl border border-slate-200 bg-slate-50 hover:bg-emerald-50 hover:border-emerald-200 transition-all"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-slate-500">
                        {member.barangay || "Barangay not set"}
                      </p>
                    </div>
                    <Badge
                      className={`px-3 py-1 rounded-full text-xs font-semibold ${
                        member.primary_role === "farmer"
                          ? "bg-emerald-100 text-emerald-700 border-emerald-200"
                          : member.primary_role === "restaurant"
                            ? "bg-orange-100 text-orange-700 border-orange-200"
                            : member.primary_role === "resident"
          ...
          <Card className="p-6 rounded-2xl border border-emerald-200 bg-white/80 backdrop-blur-sm shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Latest purchases, sales and exchanges in the municipality
                </p>
              </div>
              <Link
                to="/requests"
            ...
            <div className="mt-6 space-y-3">
              {transactionsPreview.length === 0 ? (
                <p className="text-sm text-slate-500">
                  No marketplace transactions for this municipality yet.
                </p>
              ) : (
                transactionsPreview.map((transaction) => (
                  <div
                    key={transaction.id}
                    className="flex items-center gap-4 p-4 rounded-xl border border-slate-200 bg-white hover:bg-emerald-50 hover:border-emerald-200 transition-all"
                  >
                    <div
                      className={`h-10 w-10 rounded-full flex items-center justify-center ${
                        transaction.title.includes("Purchase")
                          ? "bg-blue-100 text-blue-600"
                          : transaction.title.includes("Trade")
        ...
        <Card className="mt-8 p-6 rounded-2xl border border-emerald-200 bg-white/80 backdrop-blur-sm shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Real-time system metrics for {monitoringStats.municipalityLabel}
              </p>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-5">
            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50">
              <div className="flex items-center justify-between mb-2">
                ...
                <div
                  className={`h-2 w-2 rounded-full ${monitoringStats.activeListings > 0 ? "bg-green-500" : "bg-slate-400"}`}
                />
              </div>
            ...
            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50">
              <div className="flex items-center justify-between mb-2">
                ...
                <div
                  className={`h-2 w-2 rounded-full ${monitoringStats.completedTransactions > 0 ? "bg-green-500" : "bg-slate-400"}`}
                />
              </div>
              <p className="text-2xl font-bold text-slate-900">
                {monitoringStats.completedTransactions}
              </p>
            ...
            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50">
              <div className="flex items-center justify-between mb-2">
                ...
                <div
                  className={`h-2 w-2 rounded-full ${monitoringStats.totalMembers > 10 ? "bg-green-500" : monitoringStats.totalMembers > 5 ? "bg-yellow-500" : "bg-red-500"}`}
                />
              </div>
            ...
            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50">
              <div className="flex items-center justify-between mb-2">
                ...
                <div
                  className={`h-2 w-2 rounded-full ${monitoringStats.compostSales > 0 ? "bg-green-500" : "bg-slate-400"}`}
                />
              </div>
              <p className="text-2xl font-bold text-slate-900">
                ₱{monitoringStats.compostSales.toLocaleString()}
              </p>
            ...
            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50">
              <div className="flex items-center justify-between mb-2">
                ...
                <div
                  className={`h-2 w-2 rounded-full ${monitoringStats.foodWasteCollected > 0 ? "bg-green-500" : "bg-slate-400"}`}
                />
              </div>
              <p className="text-2xl font-bold text-slate-900">
                {monitoringStats.foodWasteCollected}kg
              </p>
        ...
        <Card className="mt-8 p-6 rounded-2xl border border-emerald-200 bg-white/80 backdrop-blur-sm shadow-sm">
          <h3 className="font-display text-lg font-semibold text-slate-900">
            Municipality-only access
          </h3>
          <p className="mt-2 text-sm text-slate-500">
            This admin workspace only shows users, listings, transactions and reports that belong to{" "}
            {monitoringStats.municipalityLabel}.
          </p>
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-sm text-slate-500">Members</p>
              <p className="mt-2 text-2xl font-semibold text-slate-900">
                {monitoringStats.totalMembers}
              </p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-sm text-slate-500">Listings</p>
              <p className="mt-2 text-2xl font-semibold text-slate-900">
                {monitoringStats.activeListings}
              </p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-sm text-slate-500">Pending requests</p>
              <p className="mt-2 text-2xl font-semibold text-slate-900">
                {monitoringStats.pendingRequests}
              </p>
            </div>
          </div>
        </Card>
