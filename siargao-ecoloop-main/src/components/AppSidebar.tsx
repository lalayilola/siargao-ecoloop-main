...
import { Leaf, LayoutDashboard, Newspaper, Store, Calendar, ArrowLeftRight, User as UserIcon, LogOut, Home, MessageCircle, Sprout, TreePine, Recycle, Bell, Megaphone, FileText } from "lucide-react";
import { useAuth } from "@/hooks/use-auth";
import { Button } from "@/components/ui/button";
import { NotificationBell } from "@/components/NotificationBell";

const memberItems = [
  { to: "/feed", label: "EcoFeed", icon: Newspaper },
  { to: "/marketplace", label: "Marketplace", icon: Store },
  { to: "/trades", label: "Transaction History", icon: ArrowLeftRight },
            ...
            <SidebarMenu>
              <SidebarMenuItem>
                <SidebarMenuButton asChild isActive={isActive("/profile")}>
                  <Link to="/profile" className="flex items-center gap-3 rounded-full px-3 py-2 text-slate-800 transition hover:bg-green-100 hover:text-green-700">
                    <UserIcon />
                    <span>Profile</span>
                  </Link>
                </SidebarMenuButton>
